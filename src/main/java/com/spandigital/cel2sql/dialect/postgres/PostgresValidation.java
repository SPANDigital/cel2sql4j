 ...
 * Ported from the Go {@code dialect/postgres/validation.go} implementation.
 */
final class PostgresValidation {

    /** Maximum identifier length in PostgreSQL (NAMEDATALEN - 1). */
    static final int MAX_IDENTIFIER_LENGTH = 63;

    /** Pattern for valid PostgreSQL identifiers: starts with letter or underscore, then alphanumeric or underscore. */
    ...
    static void validateFieldName(String name) throws ConversionException {
        ...
        if (name.length() > MAX_IDENTIFIER_LENGTH) {
            String detail = String.format(
                    "field name \"%s\" exceeds PostgreSQL maximum identifier length of %d characters",
