#!/usr/bin/env bash
# Pre-flight checks for cutting a cel2sql4j release.
#
# Usage:
...
# CI red on main). Soft warnings (open Dependabot PRs) print but don't fail.
set -euo pipefail
...
# 6. CI status for the current main HEAD.
# Only the `CI` workflow is release-blocking. `submit-gradle` (Automatic
# Dependency Submission) is advisory — it flakes on a GitHub-side 5xx.
echo
echo "=== CI on origin/main HEAD ==="
if command -v gh >/dev/null 2>&1; then
  HEAD_SHA="$(git rev-parse origin/main 2>/dev/null || echo "")"
  # `gh run list --commit` is unreliable, so pull recent runs and filter client-side.
  ...
  if [ -z "$ALL_RUNS" ] || [ "$ALL_RUNS" = "[]" ]; then
    ...
    # Returns 0 if the latest CI run on HEAD_SHA is success; nonzero otherwise.
    # Always prints one line per workflow it found.
...
"; then
  ...
  warn "gh CLI not installed — verify CI manually at https://github.com/SPANDigital/cel2sql4j/actions"
fi

# 7. Open Dependabot PRs (security ones especially shouldn't be left behind).
echo
...
# 8. Validate version (if supplied) and print the tag commands.
if [ -n "$VERSION" ]; then
  echo
  echo "=== Version check ==="
  STRIPPED="${VERSION#v}"
